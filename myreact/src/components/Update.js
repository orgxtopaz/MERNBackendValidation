...
function Update() {
  ...
  const updateUser = (e) => {
    ...
    Axios.put(`http://localhost:5000/update/${defaultId}`, {
      id: defaultId,
      email: email,
      location: location,
      ...
      .then((response) => {
        ...
        function cancel() {
          Axios.put(`http://localhost:5000/updatecancel/${defaultId}`, {
            id: defaultId,
            email: defaultEmail,
  ...
  useEffect(() => {
    if (isLoaded) {
      Axios.get(`http://localhost:5000/view/${defaultId}`).then((response) => {
